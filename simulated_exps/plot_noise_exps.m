function out = plot_noise_exps(results, noise_levels, exp_type)

n_exps = size(results,1);
n_noise = length(noise_levels);
K = length(results{1});

accs = zeros(3, n_noise, K, n_exps);
% precs = zeros(5, n_noise, K, n_exps);
% recalls = zeros(5, n_noise, K, n_exps);
% specs = zeros(5, n_noise, K, n_exps);
errs = zeros(3, n_noise, K, n_exps);
fmss = zeros(3, n_noise, K, n_exps);
for i=1:n_exps
    for j = 1:n_noise
        for k = 1:K
...
%             accs(3,j,k,i) = results{i,1,j,1}(k).cmtf.accuracy;
            accs(3,j,k,i) = results{i,1,j,1}(k).acmtf.accuracy;
...
%             errs(5,j,k,i) = mean(results{i,1,j,1}(k).acmtf_sc.err(:,1));
            if isfield( results{i,1,j,1}(k), 'cp')
                fmss(1,j,k,i) = mean(results{i,1,j,1}(k).cp.fms(:));
                fmss(2,j,k,i) = mean(results{i,1,j,1}(k).pca.fms(:));
%                 fmss(3,j,k,i) = mean(results{i,1,j,1}(k).cmtf.fms(:));
                fmss(4,j,k,i) = mean(results{i,1,j,1}(k).acmtf.fms(:));
%             fmss(5,j,k,i) = mean(results{i,1,j,1}(k).acmtf_sc.fms);
            end
        end
    end
end
...
m_fmss = mean(mean(fmss,3), 4); s_fmss = std(mean(fmss,3), [], 4);

out.acc.mean = m_accs; out.acc.std = s_accs;
out.err.mean = m_errs; out.err.std = s_errs;
out.fms.mean = m_fmss; out.fms.std = s_fmss;
...
ylabel('Accuracy')
legend('CP', 'PCA', 'ACMTF', 'location', 'southeast')
% legend('CP', 'PCA', 'CMTF', 'ACMTF', 'location', 'northeast')
% savefig(['cls_perf_',exp_type,'.fig'])
...
% % legend('CP', 'PCA', 'CMTF', 'ACMTF', 'ACMTF-SC', 'location', 'southeast')
% legend('CP', 'PCA', 'CMTF', 'ACMTF', 'location', 'southeast')
% % savefig(['res_err_',exp_type,'.fig'])
% 
% 
% if isfield(results{1}(1), 'cp')
%     figure;
%     b = bar(noise_levels, m_fmss');
%     hold
%     for i=1:length(b)
%         for k=1:size(m_accs,2)
%             errorbar(b(i).XData(k)+b(i).XOffset, m_fmss(i,k), s_fmss(i,k),'k')
%         end
%     end
%     % errorbar(noise_levels, m_specs(1,:), s_specs(1,:), '-*', 'LineWidth', 2, 'DisplayName', 'CP')
%     % hold
...
%     ax.FontSize = 17;
%     xlabel('Noise Levels (SNR)')
%     ylabel('Factor Match Scores')
%     % legend('CP', 'PCA', 'CMTF', 'ACMTF', 'ACMTF-SC', 'location', 'southeast')
%     legend('CP', 'PCA', 'CMTF', 'ACMTF', 'location', 'southeast')
%     % savefig(['fms_',exp_type,'.fig'])
% end
end