function  [X, A] = create_soft_coupled(varargin)
...
dist_coupled = params.Results.dist_coupled; % coupling distance between factors in terms of the variance of noise.
...
mode_list = cell2mat(modes);
sz = sz(mode_list);
nb_modes = length(mode_list);
...
% couple the shared modes
unique_modes = unique(mode_list);
nb_unique_modes = length(unique_modes);
for n = 1:nb_unique_modes
    id = find(mode_list==unique_modes(n));
    length_id = length(id);
    temp = A{id(1)}./length_id;
    for i = 2:length_id
        temp = temp + A{id(i)}./length_id;
    end
    for i = 1:length_id
        A{id(i)} = temp+dist_coupled/length_id*randn(size(temp));
        for r =1:Rtotal
            A{id(i)}(:,r) = A{id(i)}(:,r)/norm(A{id(i)}(:,r));
        end
    end
end

%% Generate data blocks
n_modes = cellfun(@length, modes);
n_modes = cumsum(n_modes);
P  = length(modes);
X  = cell(P,1);
X{1} = full(ktensor(lambdas{1}',A(1:n_modes(1))));
N    = tensor(randn(size(X{1})));
X{1} = X{1} + nlevel*norm(X{1})*N/norm(N);
for p = 2:P
    X{p} = full(ktensor(lambdas{p}',A((1+n_modes(p-1)):n_modes(p))));
