...
for i_e = 1:n_exps
    for i_d = 1:n_dists
        for i_r = 1:n_ranks
            ...
            data = tester_cmtf_missing('R',ranks(i_r),'flag_soft',0,...
                'dist_coupled',dists(i_d),'rnd_seed', i_e, 'M', miss_rates,...
                'noise', noise_level, 'size', sizes, 'modes', modes,...
                'lambdas', lambdas);
            ...
            % Soft coupled data - hard coupled algorithm
            data = tester_acmtf_missing('R',ranks(i_r),'flag_soft',0,...
                'dist_coupled',dists(i_d), 'rnd_seed', i_e, 'M', miss_rates,...
                'noise', noise_level, 'size', sizes, 'modes', modes,...
                'lambdas', lambdas);
            ...
            if i_d == 1
                X = zeros(size(data.X{1}));
                X(double(data.W{1})~=1) = data.X{1}(find(data.W{1}~=1));
                X = tensor(X);
                P = data.W{1};
